...
<html>
    <head>
        ...
        <link href="<?= asset('app/lib/angular/angular-datepicker.css') ?>" rel="stylesheet">
    ...
    <body ng-controller="albumesController">
        <div class="preloader">
            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    <ul class="nav navbar-nav navbar-right main-nav">
                        ...
                        <li><a href="#" data-toggle="modal" data-target="#modal1" class="btn btn-white">Crear Album</a></li>
                    </ul>
                </div>
                <!-- /.navbar-collapse -->
            </div>
                    ...
                    <div class="col-md-6 col-md-offset-3">
                        ...
                        <table class="table table-responsive">
                            ...
                            <tr ng-repeat="album in albumes">
                                <td>{{  album.id }}</td>
                                <td>{{ album.titulo }}</td>
                                <td>{{ album.fecha | date : mediumDate }}</td>
                                <td>
                                    <button class="btn btn-danger btn-sm btn-delete" ng-click="confirmDelete(album.id)">Eliminar</button>
                                    <button class="btn btn-white btn-sm" ng-click="toggle('edit', album.id)">Editar</button>
                                </td>
                            </tr>
                            </tbody>
                        </table>
        ...
        <div class="modal fade" id="modal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content modal-popup">
                    ...
                    <form name="frmAlbum" action="" class="popup-form" novalidate="">
                        <div class="form-group error">
                            <div class="col-sm-12">
                                <input type="text" class="form-control form-white has-error" id="titulo" name="titulo" placeholder="Titulo del album" value="{{titulo}}"
                                       ng-model="album.titulo" ng-required="true">
                                <span class="help-inline"
                                      ng-show="frmAlbum.titulo.$invalid && frmAlbum.titulo.$touched">Titulo es obligatorio</span>
                            </div>
                        </div>

                        <div class="form-group error">
                            <div class="col-sm-12">
                                <input type="text" class="form-control form-white has-error" id="fecha" name="fecha" placeholder="Fecha publicacion (dd/mm/yyyy)" value="{{fecha}}"
                                       ng-model="album.fecha" ng-required="true">
                                <span class="help-inline"
                                      ng-show="frmAlbum.fecha.$invalid && frmAlbum.fecha.$touched">La fecha es obligatoria</span>
                            </div>
                        </div>

                        {{album}}
                        <button type="button" class="btn btn-submit" id="btn-save" ng-click="save(modalstate, id)" ng-disabled="frmAlbum.$invalid">{{form_button}}</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script src="<?= asset('app/lib/angular/angular-datepicker.min.js') ?>"></script>
    ...
    <script src="<?= asset('app/controllers/albumesController.js') ?>"></script>
</html>
